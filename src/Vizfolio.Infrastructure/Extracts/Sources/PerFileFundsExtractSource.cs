...
public sealed class PerFileFundsExtractSource : IFundsExtractSource, IDisposable
{
    private readonly HttpClient _http;
    private readonly GitHubExtractOptions _options;
    private readonly ILogger<PerFileFundsExtractSource> _logger;
    private readonly RateLimiter _limiter;

    public PerFileFundsExtractSource(
        HttpClient http,
        IOptions<GitHubExtractOptions> options,
        ILogger<PerFileFundsExtractSource> logger)
    {
        _http = http;
        _options = options.Value;
        _logger = logger;

        var perSecond = Math.Max(1, _options.Sources.Funds.RequestsPerSecond);
    ...
    private async Task<RateLimitLease> AcquireAsync(CancellationToken cancellationToken)
    {
        ...
        if (!lease.IsAcquired)
            throw new InvalidOperationException("Could not acquire a rate-limit lease for the funds extracts source.");
        return lease;
    }
